...
// Default ALMA palette tailored for the "O que fica depois da virada" light/sand section
// u_colors[0] is background, u_colors[1..3] are the halftone dots
const ALMA_UNIFORMS = {
  colors: [
    [0.984, 0.957, 0.914], // #FDF4E9 - Fundo creme suave / papel areia claro
    [0.945, 0.824, 0.655], // #F1D2A7 - Bege areia ALMA
    [0.871, 0.725, 0.537], // #DEBA8A - Bege mais escuro / areia dourada
    [0.965, 0.878, 0.718], // #F7E0B7 - Bege suave intermediário
    [0.984, 0.957, 0.914],
    [0.984, 0.957, 0.914],
    [0.984, 0.957, 0.914],
    [0.984, 0.957, 0.914],
...
export function ShaderBackground({ className, colors: customColors }: ShaderBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const paletteColors = customColors || ALMA_UNIFORMS.colors
    const colorArray = new Float32Array(8 * 3)
    paletteColors.slice(0, 8).forEach((col, i) => {
      colorArray[i * 3] = col[0]
      colorArray[i * 3 + 1] = col[1]
      colorArray[i * 3 + 2] = col[2]
    })

    gl.uniform3fv(uni.colors, colorArray)
    gl.uniform4f(
      uni.shape,
      ALMA_UNIFORMS.scale,
      ALMA_UNIFORMS.intensity,
      ALMA_UNIFORMS.paramA,
      ALMA_UNIFORMS.warp,
    )
    gl.uniform4f(
      uni.surface,
      ALMA_UNIFORMS.detail,
      ALMA_UNIFORMS.contrast,
      ALMA_UNIFORMS.brightness,
      ALMA_UNIFORMS.saturation,
    )
    gl.uniform4f(
      uni.finish,
      ALMA_UNIFORMS.hue,
      ALMA_UNIFORMS.vignette,
      ALMA_UNIFORMS.blur,
      ALMA_UNIFORMS.grain,
    )
    gl.uniform4f(
    ...
    gl.uniform4f(
      uni.cursor,
      0,
      ALMA_UNIFORMS.cursorEffect,
      ALMA_UNIFORMS.cursorStrength,
      ALMA_UNIFORMS.cursorRadius,
    )
    ...
    const timeAnimated = Math.abs(ALMA_UNIFORMS.timeScale) > 0.0001
    ...
    if (ALMA_UNIFORMS.cursorEnabled) {
    ...
    function render(now: number) {
      ...
      gl.uniform4f(
        ...
        width,
        height,
        ((now - start) / 1000) * ALMA_UNIFORMS.timeScale,
        ALMA_UNIFORMS.colorCount,
      )
      gl.uniform4f(
        uni.space,
        ALMA_UNIFORMS.offsetX,
        ALMA_UNIFORMS.offsetY,
        mouseX,
        mouseY,
      )
      gl.uniform4f(
        uni.cursor,
        ALMA_UNIFORMS.cursorEnabled ? cursorPresence : 0,
        ALMA_UNIFORMS.cursorEffect,
        ALMA_UNIFORMS.cursorStrength,
        ALMA_UNIFORMS.cursorRadius,
      )
